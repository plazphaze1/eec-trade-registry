...
export default async function FinancialAccountPage({ params, searchParams }: AccountPageProps) {
  ...
  return <main className="staff-main bank-account-detail">
    ...
    {(capabilities.can_post || capabilities.can_manage_accounts) && account.account_type !== "company_treasury" && <section className="bank-quick-actions">
      {capabilities.can_post && account.status === "active" && <details><summary><strong>Place a hold</strong><span>Temporarily reserve part of the available balance</span></summary><form action={placeAccountHoldAction} className="bank-action-form"><CommandRequestId /><input name="account_id" type="hidden" value={account.id}/><label><span>Amount</span><input max={account.available_balance_minor} min="1" name="amount_minor" required step="1" type="number"/></label><label><span>Reason</span><input name="reason" required/></label><label><span>Expires optional</span><input name="expires_at" type="datetime-local"/></label><button className="button button-primary">Place hold</button></form></details>}
      {capabilities.can_manage_accounts && account.status !== "closed" && <details><summary><strong>{account.status === "frozen" ? "Unfreeze account" : "Freeze or close"}</strong><span>Stop movement without changing the recorded balance</span></summary><form action={setAccountStatusAction} className="bank-action-form"><CommandRequestId /><input name="account_id" type="hidden" value={account.id}/><input name="expected_version" type="hidden" value={account.version}/><label><span>New status</span><select defaultValue={account.status === "frozen" ? "active" : "frozen"} name="status"><option value="active">Active</option><option value="frozen">Frozen</option><option value="closed">Closed</option></select></label><label><span>Reason</span><input name="reason" required/></label><button className="button button-primary">Update account</button></form></details>}
    </section>}

    <section className="bank-two-column">
      <div className="bank-panel"><div className="bank-panel-heading"><div><p className="eyebrow">Reserved money</p><h2>Holds</h2></div><span>{activeHolds.length} active</span></div>{holds.length ? <div className="bank-list">{holds.map((hold) => <article key={hold.id}><div><strong>{hold.reason}</strong><span>{hold.public_reference} · {hold.status}{hold.expires_at ? ` · until ${new Date(hold.expires_at).toLocaleString()}` : ""}</span></div><div><strong>{amount(hold.amount_minor, account.currency_code)}</strong>{hold.status === "active" && capabilities.can_post && <form action={releaseAccountHoldAction} className="bank-inline-form"><CommandRequestId /><input name="account_id" type="hidden" value={account.id}/><input name="expected_version" type="hidden" value={hold.version}/><input name="hold_id" type="hidden" value={hold.id}/><input name="reason" type="hidden" value="Released from the account statement."/><button className="button button-secondary">Release</button></form>}</div></article>)}</div> : <p className="empty-state">No holds have been placed.</p>}</div>
