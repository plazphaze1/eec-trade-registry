...
const errors: Record<string, string> = {
  ...
  concurrent_update: "Another money action finished at the same time. Nothing was duplicated; try once more.",
  ...
  same_account: "Choose two different accounts for a transfer.",
...
function Transactions({ scope, workspace }: { scope: "bank" | "books"; workspace: BankingWorkspace }) {
  ...
  return <>
    {scope === "bank" && workspace.capabilities.can_post && <section className="bank-quick-actions">
      <details open><summary><strong>Deposit or withdraw</strong><span>Cash entering or leaving one account</span></summary><form action={cashMovementAction} className="bank-action-form"><CommandRequestId /><div className="bank-form-row"><label><span>Action</span><select name="direction"><option value="deposit">Deposit</option><option value="withdrawal">Withdraw</option></select></label><label><span>Account</span><select name="account_id" required>{accounts.map((account) => <option key={account.id} value={account.id}>{account.display_name} · {amount(account.available_balance_minor, account.currency_code)}</option>)}</select></label></div><div className="bank-form-row"><label><span>Amount</span><input min="1" name="amount_minor" required step="1" type="number" /></label><label><span>Date</span><input defaultValue={today()} name="occurred_on" required type="date" /></label></div><label><span>What was this for?</span><input name="memo" required /></label><label><span>Receipt or reference optional</span><input name="reference" /></label><button className="button button-primary">Post money movement</button></form></details>
      {accounts.length > 1 && <details open><summary><strong>Transfer between accounts</strong><span>Move Septims from one account to another</span></summary><form action={transferAction} className="bank-action-form"><CommandRequestId /><div className="bank-form-row"><label><span>From</span><select name="from_account_id" required>{accounts.map((account) => <option key={account.id} value={account.id}>{account.display_name} · {amount(account.available_balance_minor, account.currency_code)}</option>)}</select></label><label><span>To</span><select defaultValue="" name="to_account_id" required><option disabled value="">Choose destination account</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.display_name}</option>)}</select></label></div><div className="bank-form-row"><label><span>Amount</span><input min="1" name="amount_minor" required step="1" type="number" /></label><label><span>Date</span><input defaultValue={today()} name="occurred_on" required type="date" /></label></div><label><span>What was this for?</span><input name="memo" required /></label><label><span>Reference optional</span><input name="reference" /></label><button className="button button-primary">Transfer Septims</button></form></details>}
    </section>}
    <section className="bank-panel"><div className="bank-panel-heading"><div><p className="eyebrow">Immutable journal</p><h2>{scope === "books" ? "Company ledger" : "Bank activity"}</h2><p>Corrections create a reversal. Posted rows are never rewritten.</p></div><span>{transactions.length} recent entries</span></div><div className="bank-table-wrap"><table className="bank-table"><thead><tr><th>Date</th><th>Transaction</th><th>From → To</th><th>Amount</th><th>Source</th></tr></thead><tbody>{transactions.map((transaction) => <tr key={transaction.id}><td>{transaction.occurred_on}</td><td><strong>{transaction.memo}</strong><small>{transaction.public_reference} · {transaction.transaction_type.replaceAll("_", " ")}{transaction.is_reversed ? " · reversed" : ""}</small></td><td>{transaction.from_account}<small>→ {transaction.to_account}</small></td><td>{amount(transaction.amount_minor, transaction.currency_code)}</td><td>{transaction.source_reference ?? transaction.external_reference ?? "Manual"}</td></tr>)}</tbody></table></div>{!transactions.length && <p className="empty-state">No money has moved yet. Use the form above to record the first transaction.</p>}</section>
  </>;
}
...
function Controls({ controls, workspace }: { controls: BankingControls; workspace: BankingWorkspace }) {
  ...
  return <>
    ...
    <section className="bank-panel">
      ...
      <div className="bank-table-wrap"><table className="bank-table"><thead><tr><th>Period</th><th>Dates</th><th>Accounts captured</th><th>Status</th><th>Action</th></tr></thead><tbody>{controls.periods.map((period) => <tr key={period.id}><td><strong>{period.public_reference}</strong><small>{period.note || "No note"}</small></td><td>{period.starts_on}<small>through {period.ends_on}</small></td><td>{period.account_count}</td><td><span className={`money-status is-${period.status}`}>{period.status}</span></td><td>{period.status === "closed" && controls.can_manage ? <details className="bank-table-action"><summary>Reopen</summary><form action={reopenFinancialPeriodAction} className="bank-action-form"><CommandRequestId /><input name="period_id" type="hidden" value={period.id}/><input name="expected_version" type="hidden" value={period.version}/><label><span>Why must it reopen?</span><input name="reason" required /></label><button className="button button-secondary">Reopen period</button></form></details> : "—"}</td></tr>)}</tbody></table></div>
