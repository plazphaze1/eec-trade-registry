...
export default async function LoanPage({ params, searchParams }: LoanPageProps) {
  ...
  return <main className="staff-main bank-loan-detail">
    ...
    <section className="bank-quick-actions">
      {capabilities.can_post && ["active", "defaulted"].includes(loan.status) && remaining > 0 && <details><summary><strong>Record repayment</strong><span>Allocates fees, interest, then principal to the oldest balance</span></summary><form action={recordLoanPaymentAction} className="bank-action-form"><CommandRequestId /><input name="loan_id" type="hidden" value={loan.id}/><label><span>Amount</span><input max={remaining} min="1" name="amount_minor" required step="1" type="number"/></label><label><span>Date</span><input defaultValue={today()} name="occurred_on" required type="date"/></label><label><span>Payment reference optional</span><input name="payment_reference"/></label><input name="note" type="hidden" value={`Loan payment ${loan.public_reference}`}/><button className="button button-primary">Post repayment</button></form></details>}
      {capabilities.can_manage && loan.status !== "paid" && <details><summary><strong>Change loan status</strong><span>Mark default, restore active servicing, or write off</span></summary><form action={setLoanStatusAction} className="bank-action-form"><CommandRequestId /><input name="expected_version" type="hidden" value={loan.version}/><input name="loan_id" type="hidden" value={loan.id}/><label><span>New status</span><select name="status"><option value="active">Active</option><option value="defaulted">Defaulted</option><option value="written_off">Written off</option></select></label><label><span>Reason</span><input name="reason" required/></label><button className="button button-primary">Update loan</button></form></details>}
    </section>
