...
export function PublicHeader({ institutionName }: { institutionName: string }) {
  ...
  return <header className="site-header"><div className="site-header-inner">
    <Link className="brand" href="/" aria-label={`${institutionName} home`}><span className="brand-mark" aria-hidden="true"><EecLogo priority sizes="56px" /></span><span><strong>{institutionName}</strong><small>Trade &amp; supply</small></span></Link>
...
export function PublicFooter({ institutionName }: { institutionName: string }) {
  const pathname = usePathname();
  if (pathname.startsWith("/staff") || pathname.startsWith("/dealer")) return null;
  return <footer className="site-footer"><EecLogo className="footer-logo" sizes="64px"/><div><strong>{institutionName}</strong><p>Goods, business licensing, and official verification in one place.</p><Link href="/how-it-works">How the system works</Link></div><p>Catalogue information comes directly from the Company registry. Final price and availability are confirmed when an order is placed.</p></footer>;
}
