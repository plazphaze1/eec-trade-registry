...
select lives_ok($test$
  select * from public.staff_post_cash_movement(
    (select id from public.financial_accounts where account_type='company_treasury' limit 1),
...
$test$, 'Owner records Treasury funds');
...
select throws_ok(format($test$
  select * from public.staff_transfer_funds(%L::uuid,
    (select id from public.financial_accounts where account_type='company_treasury' limit 1),2800,current_date,
    'Too much while held.',null,'Test hold enforcement.','ba300000-0000-4000-8000-000000000004')
$test$,current_setting('test.bank_account_id')),'23514','financial_insufficient_funds','holds prevent overspending');
select lives_ok($test$
  select * from public.staff_release_account_hold(
    (select id from public.financial_account_holds where source_request_id='ba300000-0000-4000-8000-000000000003'),1,
    'Trade cancelled.','ba300000-0000-4000-8000-000000000005')
$test$,'hold can be released');
reset role;
...
select lives_ok(format($test$
  select * from public.staff_record_invoice_payment(
    (select id from public.sales_invoices where order_id='ba400000-0000-4000-8000-000000000001'),%L::uuid,
    400,current_date,'PAY-ONE','First payment.','Record first partial payment.','ba300000-0000-4000-8000-000000000007')
$test$,current_setting('test.bank_account_id')),'partial invoice payment posts');
select lives_ok(format($test$
  select * from public.staff_record_invoice_payment(
    (select id from public.sales_invoices where order_id='ba400000-0000-4000-8000-000000000001'),%L::uuid,
    600,current_date,'PAY-TWO','Final payment.','Record final payment.','ba300000-0000-4000-8000-000000000008')
$test$,current_setting('test.bank_account_id')),'a second payment can settle the invoice');
reset role;
...
select lives_ok(format($test$
  select * from public.staff_originate_loan(
    (select id from public.loan_products where code='standard-test'),%L::uuid,1200,6,current_date,current_date+30,
    'Working capital.','Approve tested loan.','ba300000-0000-4000-8000-000000000010')
$test$,current_setting('test.bank_account_id')),'loan approval disburses real funds');
reset role;
...
select lives_ok($test$
  select * from public.staff_record_loan_payment(
    (select id from public.loans where source_request_id='ba300000-0000-4000-8000-000000000010'),200,current_date+30,
    'LOAN-PAY-ONE','Scheduled repayment.','Record tested loan payment.','ba300000-0000-4000-8000-000000000011')
$test$,'loan repayment posts and allocates');
select throws_ok($test$
  select * from public.staff_reverse_financial_transaction(
    (select financial_transaction_id from public.loan_payments where source_request_id='ba300000-0000-4000-8000-000000000011'),
    'Unsafe generic reversal attempt.','ba300000-0000-4000-8000-000000000012')
$test$,'22023','linked_payment_requires_domain_refund','linked loan payment cannot be generically reversed');
...
select lives_ok(format($test$
  select * from public.dealer_transfer_funds(%L::uuid,
    (select public_reference from public.financial_accounts where account_type='company_treasury' limit 1),50,current_date,
    'Business transfer to Company.','ba500000-0000-4000-8000-000000000003')
$test$,current_setting('test.bank_account_id')),'business representative can transfer owned funds');
select throws_ok($test$
  select * from public.dealer_transfer_funds(
    (select id from public.financial_accounts where account_type='company_treasury' limit 1),'EEC-ACC-NOT-USED',1,current_date,
    'Unauthorized source.','ba500000-0000-4000-8000-000000000004')
$test$,'22023','dealer_source_account_invalid','business representative cannot spend another party account');
reset role;
